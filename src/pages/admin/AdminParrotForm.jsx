  ...
  useEffect(() => {
    if (isEdit) {
      adminAPI.getParrotById(id).then(r => {
        ...
        setForm({ name: c.name || '', description: c.description || '', ringNumber: c.ringNumber || '', color: c.color || '', sex: c.sex || '', birthDate: c.birthDate ? c.birthDate.split('T')[0] : '', species: c.species || '' });
        const existing = [];
        ...
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:12, padding:28, marginBottom:22, boxShadow:'var(--shadow-sm)' }}>
          <div style={{ display:'grid', gap:20, marginBottom:24 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:20 }}>
              ...
              <div>
                <label style={{ display:'block', fontSize:11, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--text-3)', marginBottom:10 }}>Couleur</label>
                <input name="color" value={form.color} onChange={set("color")} placeholder="Bleu, vert, gris..."
                  className="input-luxury" style={{ fontSize:15, borderRadius:10, padding:'14px 18px', width:'100%' }} />
              </div>
              <div>
                <label style={{ display:'block', fontSize:11, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--text-3)', marginBottom:10 }}>Sexe</label>
                <select name="sex" value={form.sex} onChange={set("sex")}
                  className="input-luxury" style={{ fontSize:15, borderRadius:10, padding:'14px 18px', width:'100%', appearance:'auto', cursor:'pointer' }}>
                  <option value="">—</option>
