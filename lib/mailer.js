...
const FROM   = `"${process.env.FROM_NAME || 'Le Parc des Perroquets'}" <${process.env.FROM_EMAIL || process.env.SMTP_USER}>`;

function getPublicSiteUrl() {
  if (process.env.APP_URL) return process.env.APP_URL.replace(/\/$/, '');
  return 'https://leparcdesperroquets.fr';
}
...
<td><span style="font-size:17px;font-weight:800;color:#2C1810;">Le Parc des Perroquets</span><br><span style="font-size:9px;letter-spacing:0.25em;color:#C9762E;text-transform:uppercase;">Bren · Drôme</span></td>
...
<strong style="color:#2C1810;">Le Parc des Perroquets</strong><br>1235 Route de Claveyson — 26260 Bren, France<br>TVA FR72498339654</span></td></tr>
</table>
</td></tr></table>
</body>
</html>`;
...
async function sendMail({ to, subject, html, text }) {
  ...
  return transporter.sendMail({
    ...
    headers: {
      ...
      'X-Mailer': 'Parc des Perroquets Mailer v1',
...
async function sendReservationConfirmation({ email, name, reservation, parrot }) {
  const pn = parrot.name + ' · ' + parrot.species;
  ...
  const html = wrapHtml('Réservation confirmée',
    ...
    L('Perroquet', pn) +
    L('Prix', formatEuro(parrot.price)) +
    SEP() +
    ...
    L('Total', formatEuro(reservation.totalPrice || parrot.price)) +
  ...
  await sendMail({
    to: email,
    subject: `Réservation ${reservation.reservationNumber} — Le Parc des Perroquets`,
    html,
    text: [
      `Réservation confirmée — Le Parc des Perroquets`,
      ``,
      `Bonjour ${name},`,
      `Votre réservation ${reservation.reservationNumber} a bien été enregistrée.`,
      ``,
      `Perroquet : ${parrot.name} (${parrot.species})`,
      `Prix : ${formatEuro(parrot.price)}`,
      ``,
      ...
      `Total : ${formatEuro(reservation.totalPrice || parrot.price)}`,
...
async function sendAdminNotification({ reservation, parrot }) {
  ...
  const html = wrapHtml('Nouvelle réservation',
    ...
    L('Livraison', dm) +
    SEP() +
    L('Perroquet', parrot.name + ' (' + parrot.species + ')') +
    L('Prix', formatEuro(parrot.price)) +
    L('Paiement', payLabel) +
...
async function sendStatusNotification({ email, name, reservationNumber, status, parrot }) {
  const statusLabels = {
  ...
  const html = wrapHtml('Mise à jour',
    ...
    (parrot ? L('Concernant', esc(parrot.name) + ' (' + esc(parrot.species) + ')') : '')
  );
  ...
  await sendMail({
    ...
    text: [
      ...
      ...(parrot ? [`Concernant : ${parrot.name} (${parrot.species})`] : []),
      ``,
      `-- Le Parc des Perroquets`,
      `1235 Route de Claveyson — 26260 Bren, France`,
      `TVA FR72498339654`,
...
async function sendReplyToCustomer({ email, name, subject, message }) {
  ...
  await sendMail({
    to: email,
    subject: subject || 'Le Parc des Perroquets — Message de votre éleveur',
    html,
    text: [
      `Message du Parc des Perroquets`,
      ``,
      `Bonjour ${name},`,
      ``,
      message,
